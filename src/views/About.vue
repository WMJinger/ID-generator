<template>
  <div class="about-page" itemscope itemtype="https://schema.org/WebPage">
      ...
      <section class="page-header" aria-labelledby="page-title">
        <div class="breadcrumb" itemscope itemtype="https://schema.org/BreadcrumbList">
          ...
          <span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem" class="breadcrumb-current">
            <span itemprop="name">{{ t('aboutUs', currentCountry) }}</span>
        ...
        <h1 id="page-title" class="page-title" itemprop="headline">
          ...
          {{ t('aboutUs', currentCountry) }}
        </h1>
      </section>

      <section class="about-content">
        <article class="about-card">
          <h2>{{ t('whoWeAre', currentCountry) }}</h2>
          <p>{{ t('aboutDescription1', currentCountry) }}</p>
          <p>{{ t('aboutDescription2', currentCountry) }}</p>
        </article>

        <article class="about-card">
          <h2>{{ t('ourMission', currentCountry) }}</h2>
          <p>{{ t('missionDescription', currentCountry) }}</p>
        </article>

        <article class="about-card">
          <h2>{{ t('coreValues', currentCountry) }}</h2>
          <ul>
            <li><strong>{{ t('valuePrivacy', currentCountry) }}</strong></li>
            <li><strong>{{ t('valueSecurity', currentCountry) }}</strong></li>
            <li><strong>{{ t('valueEasy', currentCountry) }}</strong></li>
            <li><strong>{{ t('valueImprove', currentCountry) }}</strong></li>
          </ul>
        </article>

        <article class="about-card">
          <h2>{{ t('contactUsTitle', currentCountry) }}</h2>
          <p>{{ t('contactDescription', currentCountry) }}</p>
          <p>📧 {{ t('email', currentCountry) }}: {{ t('emailAddress', currentCountry) }}</p>
        </article>
      </section>
    </main>

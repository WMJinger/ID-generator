<template>
  <div class="terms-page" itemscope itemtype="https://schema.org/WebPage">
      ...
      <section class="page-header" aria-labelledby="page-title">
        <div class="breadcrumb" itemscope itemtype="https://schema.org/BreadcrumbList">
          ...
          <span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem" class="breadcrumb-current">
            <span itemprop="name">{{ t('termsOfService', currentCountry) }}</span>
        ...
        <h1 id="page-title" class="page-title" itemprop="headline">
          ...
          {{ t('termsOfService', currentCountry) }}
        </h1>
      </section>

      <section class="terms-content">
        <article class="terms-card">
          <h2>{{ t('termsOfService', currentCountry) }}</h2>
          <p>{{ t('lastUpdated', currentCountry) }}: {{ t('lastUpdatedDate', currentCountry) }}</p>
        </article>

        <article class="terms-card">
          <h2>{{ t('acceptTerms', currentCountry) }}</h2>
          <p>{{ t('acceptTermsDesc', currentCountry) }}</p>
        </article>

        <article class="terms-card">
          <h2>{{ t('serviceUsage', currentCountry) }}</h2>
          <h3>{{ t('license', currentCountry) }}</h3>
          <p>{{ t('licenseDesc', currentCountry) }}</p>
          
          <h3>{{ t('usageRestrictions', currentCountry) }}</h3>
          <p>{{ t('usageRestrictionsDesc', currentCountry) }}</p>
          <ul>
            <li>{{ t('restrictIllegal', currentCountry) }}</li>
            <li>{{ t('restrictAbuse', currentCountry) }}</li>
            <li>{{ t('restrictHack', currentCountry) }}</li>
            <li>{{ t('restrictCopy', currentCountry) }}</li>
          </ul>
        </article>

        <article class="terms-card">
          <h2>{{ t('generatedData', currentCountry) }}</h2>
          <p>{{ t('generatedDataDesc', currentCountry) }}</p>
          <p>{{ t('generatedDataDesc2', currentCountry) }}</p>
        </article>

        <article class="terms-card">
          <h2>{{ t('disclaimer', currentCountry) }}</h2>
          <p>{{ t('disclaimerDesc', currentCountry) }}</p>
        </article>

        <article class="terms-card">
          <h2>{{ t('termsChanges', currentCountry) }}</h2>
          <p>{{ t('termsChangesDesc', currentCountry) }}</p>
        </article>

        <article class="terms-card">
          <h2>{{ t('contactUsTitle', currentCountry) }}</h2>
          <p>{{ t('termsContact', currentCountry) }} {{ t('emailAddress', currentCountry) }}</p>
        </article>
      </section>
    </main>

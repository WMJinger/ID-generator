<template>
  <div class="generator-card">
    <h2 class="card-title">{{ t('generateSocialCreditCodeTitle', currentCountry) }}</h2>
    ...
    <el-form label-position="top" class="generator-form">
      ...
      <el-form-item :label="t('socialCreditCode', currentCountry)">
        <el-input v-model="result" :placeholder="t('socialCreditCodePlaceholder', currentCountry)" readonly>
          <template #append>
...
import { generateSocialCreditCode, validateSocialCreditCode } from '@/utils/generator';
...
function generate() {
  ...
  result.value = generateSocialCreditCode();
  analytics.trackToolUse('socialCreditCode', 'generate');
}

function validate() {
  if (!result.value) return;

  const isValid = validateSocialCreditCode(result.value);
  validationResult.value = isValid ? 'valid' : 'invalid';
  analytics.trackToolUse('socialCreditCode', 'validate');
}
...
async function copyResult() {
  if (result.value) {
    ...
    analytics.trackToolUse('socialCreditCode', 'copy');
  }
}
</script>
